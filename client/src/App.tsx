...
const SERVER_HOME = "https://aiva-ddmx.onrender.com/"; 
...
export default function App() {
  ...
  useEffect(() => {
    if (loading) {
      const texts = ["Consulting Llama-3...", "Structuring concept...", "Applying Feynman Method...", "Almost ready..."];
      let i = 0;
  ...
  useEffect(() => { localStorage.setItem('study_vault', JSON.stringify(history)); }, [history]);
  ...
  const handleGenerate = async (forcedTopic?: string) => {
    ...
    try {
      ...
      const aiText = data.result || "I'm having trouble connecting to my brain right now.";
      ...
      setActiveChat(prev => [...prev, { 
        ...
        role: 'aiva', 
        text: "⚠️ **Server is warming up.** Render's free tier takes about 50 seconds to boot if it's been idle. Please wait a moment and send your message again!" 
      }]);
      setServerStatus('sleeping');
    } finally { 
      setLoading(false); 
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-80 bg-white border-r border-slate-200 flex flex-col hidden lg:flex">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          <div className={`space-y-2 transition-all duration-500 ${isDeleting ? 'opacity-0 scale-95' : 'opacity-100'}`}>
            {history.map((item) => (
        ...
        {!hasStarted ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-6 animate-in fade-in duration-700">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-2xl w-full mb-10">
                <button onClick={() => handleGenerate("Explain photosynthesis with Feynman Technique")} className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:shadow-md transition-all group text-left">
                  ...
                  <span className="font-bold text-[10px] text-slate-600 uppercase tracking-tighter">Concept<br/>Mentor</span>
                </button>
                <button onClick={() => handleGenerate("Summarize the French Revolution")} className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:shadow-md transition-all group text-left">
                  ...
                  <span className="font-bold text-[10px] text-slate-600 uppercase tracking-tighter">Memory<br/>Architect</span>
                </button>
                <button onClick={() => handleGenerate("Quiz me on basic biology")} className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:shadow-md transition-all group text-left">
                  ...
                  <span className="font-bold text-[10px] text-slate-600 uppercase tracking-tighter">Quiz<br/>Master</span>
                </button>
             </div>
          ...
          <>
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-6 flex flex-col items-center">
              <div className="w-full max-w-3xl space-y-8 pb-48">
                {activeChat.map((msg) => (
                  <div key={msg.id} className={`flex items-start gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-in slide-in-from-bottom-2`}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-slate-50 via-slate-50 to-transparent">
              <div className="max-w-3xl mx-auto flex flex-col gap-2 bg-white p-2 rounded-[2rem] shadow-2xl border border-slate-200">
                <textarea 
                  rows={1}
